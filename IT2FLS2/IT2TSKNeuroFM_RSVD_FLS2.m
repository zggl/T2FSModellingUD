            ...
            if J==0 % The first  round
                ...
                if ~isempty(indGj)% passes the input similarity test
                    ...
                    if ~isempty(col)                % passes the output similarity test
                        ...
                        Net(2).m{1}(:,J)=(m_t1*St+In_i)/(St+1);
                        Net(2).m{2}(:,J)=(m_t2*St+In_i)/(St+1);
                    ...
                    if J < J_max %  generate a new rule
                        ...
                        St=Net(1).Sj(Gt_ind);
                        Net(2).m{1}(:,Gt_ind)=(Net(2).m{1}(:,Gt_ind)*St+In_i)/(St+1);
                        Net(2).m{2}(:,Gt_ind)=(Net(2).m{2}(:,Gt_ind)*St+In_i)/(St+1);
...
function [MSE,Net]=RSVD(N,In,inds,Out,J,J_max,In_dim,Net)
