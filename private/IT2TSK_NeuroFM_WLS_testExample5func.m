function [Results,Net1]=IT2TSK_NeuroFM_WLS_testExample5func(J_max,Iter,In,Out,nn,Net)
...
Results=[];
for ite=1:Iter
    Net2{ite}=IT2TSKNeuroFM_WLS(Net,QBFAStruc,X_scaled,Net(1).N0,Net(1).Block,Net(1).C,Net(1).W(Net(1).N0));
    Results(ite,:)=[Net2{ite}(4).MSE,sqrt(Net2{ite}(4).MSE)];   
end
[~,ind]=min(Results(:,1));
Net1=Net2{ind};
% Net(2).m{1}
